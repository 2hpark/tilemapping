...
// ============================================================
const SCREEN_TITLE  = "title.js";
const SCREEN_A      = "sketch.js";
...
// ============================================================
// DRAW
// One dispatch point: look at currentScreen, call the matching
// draw function. Add an "else if" per new screen.
// ============================================================
function draw() {
  background(20);
  ...
  if (currentScreen === SCREEN_TITLE) {
    drawTitleScreen();
  } else if (currentScreen === SCREEN_A) {
    drawScreenA();
  } 
}
...
// ============================================================
// SCREEN CONTENT
// One draw function per screen. Pure drawing — no state changes.
// ============================================================
...
let windTiles = []; // [{x,y,w,h}] trigger tiles for the end overlay
...
function preload() {
  ...
  cavebgImg = loadImage("assets/cavebg.png");
  titleFrame1Img = loadImage("assets/Title frame1.png");
...
function drawTiles(jsonFile) {
  ...
  // Second pass (tileData only): draw bg green tiles, then cavebg on top of them
  if (jsonFile === tileData) {
    for (let l = layers.length - 1; l > -1; l--) {
      const layer = layers[l];
    ...
    if (cavebgImg) {
      const birdAreaRight = tileData.mapWidth * TILE_SIZE;
      image(cavebgImg, birdAreaRight - cavebgImg.width, 0);
    }
  }
  ...
  // Third pass: draw all non-water layers (skip bg green for tileData — already drawn above)
  for (let l = layers.length - 1; l > -1; l--) {
    // for each layer we will....
    const layer = layers[l];
    if (layer.name === "water") continue; // skip water, already drawn
    if (jsonFile === tileData && layer.name === "bg green") continue; // already drawn in pass 2
